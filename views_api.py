...
async def get_tunnel(user: User = Depends(check_user_exists)) -> TunnelResponse:
...
async def create_tunnel(req: TunnelRequest, user: User = Depends(check_user_exists)) -> TunnelRecord:
    return await create_or_topup(user.id, req.days)


@tunnel_router.post("/api/v1/tunnel/confirm", response_model=TunnelRecord)
async def confirm_tunnel(payment_hash: str, user: User = Depends(check_user_exists)) -> TunnelRecord:
