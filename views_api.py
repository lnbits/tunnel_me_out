...
async def get_tunnel(user: User = Depends(check_super_user)) -> TunnelResponse:
    tunnel = await fetch_existing(user.id, prune_pending=True)
...
@tunnel_router.post("/api/v1/tunnel/reconnect", response_model=TunnelRecord)
async def reconnect_tunnel(user: User = Depends(check_super_user)) -> TunnelRecord:
    tunnel = await activate_tunnel(user.id)
    if not tunnel:
        raise HTTPException(status_code=404, detail="Tunnel not found")
    return tunnel


@tunnel_router.get("/api/v1/tunnel/ping")
async def ping(user: User = Depends(check_super_user)) -> dict:
    reachable = await ping_tunnel(user.id)
